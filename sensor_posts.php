<?php
    ...
    $sql = "SELECT * FROM posts WHERE isReported = 1 ORDER BY viewOrder DESC";
...
<html lang="en">
    <head>
        ...
        <title>Admin Post _BIGZ chat</title>
    </head>
    <body>
        <header>
        <nav class="nav_bar">
        ...
        <div class="post">
            <span id="post">Post: click here shift to suggestion box</span>
        </div>
        <?php
            while($post = mysqli_fetch_assoc($posts)){
        ?>
        <div class="content">
            <a target="_blank" href="<?php echo 'sensor_reply.php?id='.$post['id'];?>" id="a_title">
            <div class="container">
                <span id="c_title"><?php echo $post['title']; ?></span>
                <div class="links">
                    <a class="link" href="<?php echo 'include/sensor_post_pass.php?id='.$post['id'];?>">pass</a>
                    <a class="link" href="<?php echo 'include/sensor_post_inappropriate.php?id='.$post['id'];?>">
                        <?php
                            if($post['inappropriate']==0){
                                echo 'inappropriate';
                            }else{
                                echo 'appropriate';
                            }
                    ...
                    ?>
                    <a class="link" href="<?php echo 'edit_post.php?isTitle=1&postid='.$post['id']; ?>" target = "_blank">edit</a>
                    <a class="link" href="<?php echo 'include/sensor_post_delete.php?id='.$post['id'];?>">delete</a>
                    <?php
                        }
                    ?>
                </div>                 
        ...
        <script src="js/sensor_posts.js"></script>

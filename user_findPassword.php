<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(!mysqli_num_rows($search)){
            echo "<script> alert('this email address is not linked to an account'); </script>";
        }else{
            $search = mysqli_fetch_assoc($search);
            $password = $search['password'];
            $username = $search['username'];
            $subject = "Find Password";
            $message = "<div>BIGZ Chat</div>\n<a href='http://bigzchats.com/user_login.php'>Login</a >\n<div>The following is your username and password</div>\n<div>username: $username</div>\n <div>password: $password</div>";
            ...
            if(mail($email,$subject,$message,$headers)){
                ...
                echo "<script> alert('failed to send the password, please try again later') </script>";
            }
        }
    }
...
<html lang="en">
    <head>
        ...
        <title>Find Password _BIGZ Chat</title>
    </head>
    <body>
        <header>
        <nav class="nav_bar">
        ...
        <form action="" method="POST" class="registration">
            <h2 class="toplog">Find your password</h2>
            <div class="body">Please enter your email below</div>
            <div></div>
            <div class="form">
                <label>Email: </label>
